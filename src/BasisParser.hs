...
basisFileParser :: MyParser st [Element]
basisFileParser = do
    header     <- basisHeader
...
basisHeader :: MyParser st Element
basisHeader = do
    char '#'
    many1 space
...
parseBasisSection :: MyParser st [Element]
...
parseBasisElement :: MyParser st Element
...
atomSection :: MyParser st (String,[GaussShape])
atomSection = do
...
atomBasisParameters :: MyParser st ([String],[String])
...
shapeParameters :: String -> MyParser st GaussShape
...
parseSingleShape :: MyParser st (Double,Double)
parseSingleShape = do
    many1 space
    eA            <- realNumber
    many1 space
...
parseSharedShape :: MyParser st (Double,Double,Double)
parseSharedShape = do
    many1 space
    eAB           <- realNumber
    many1 space
...
-- "Cl    SP"
atomSectionHeader :: MyParser st (String,String)
...
orbitalIdent :: MyParser st String 
orbitalIdent = do
    number <- intNumber
