...
type ElementParser = ABL.Result FileLine
type ParseError    = String


processFastFile :: FilePath -> IO ()
processFastFile file = do
        putStrLn $ show file
...
parseFile fname = do
    ...
    return $ ABL.eitherResult $ ABL.parse parseFastFile input

parseFastFile :: ABL.Parser [FileLine]
parseFastFile = do
    elements <- ABL.many1 parseFileLine
    ABL.endOfInput
    return $ elements

parseFileLine :: ABL.Parser FileLine
parseFileLine = do
--         parseNewLine
--     <|> parseBlankLine
--     <|> parseLine
...
parseAnyLine :: ABL.Parser FileLine
parseAnyLine = ABL.try $ do
    elements <- ABL.takeTill newLine
    return $ FileLine 10 "anyLine" [SData elements]
...
dumpLine l@(FileLine i d e) = do
    ...
    where
        ...
        dumpElement (SData x) = show x
        dumpElement (CData x) = [x]
        dumpElement (Space 0) = ""
        dumpElement (Space 1) = " "
        dumpElement (Space x) = Prelude.take x $ repeat ' '
